...
\begin{frame}
[fragile]\frametitle{Unobserved and Heterogeneous Confounders}
...
\item Treatment effect ($T$) varies according to the value of unobserved confounders ($U$).
...
\item Likely the norm in observational settings. 
% the optimal action for person A and person B might be different. 
...
Given the high-level goal of personalization, and the context of unobserved heterogeneity:
...
\begin{center}
D^*_{\text{AI}}(M) = ~$\underset{I \in {0,1}}{\mathrm{argmax}}~E[\pi|I, M]$
\end{center}
...
\begin{center}
D^*_{\text{IPVR}}(M) = $\underset{I \in {0,1}}{\mathrm{argmax}}~E[\pi| do(I), M, V=1]$
\end{center}
...
\begin{center}
D^*_{\text{IFR}}(M) = $\underset{I \in {0,1}}{\mathrm{argmax}}~E[\pi|do(I), M]$
\end{center}
...
D^*_{\text{CI}}(M, I) = $\underset{a' \in~{0,1}}{\mathrm{argmax}}~E[\pi_{a'}| I=a, M]$
...
\item A/B Tests are `gold standard' in the One-Treatment-Fits-All paradigm because they remove the influence of unobserved confounders (unmeasured variables that influence both the treatment and the outcome). 
