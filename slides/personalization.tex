...
\begin{figure}[t]
\includegraphics[width=5cm]{figures/personalizedmed.jpg}
\centering
\end{figure}
...
\small{
\begin{itemize}
\item A/B Tests are `gold standard' in the One-Treatment-Fits-All paradigm because they remove the influence of unobserved confounders (variables that influence both the treatment and the outcome). 
\end{itemize}
}
...
\textbf{In the presence of unobserved confounders (most plausible scenario), experimental data is likely not `gold standard' for estimating heterogenous treatment effects.}

\pause

\vskip20pt

In what follows, I'll use a Personalized Marketing problem as a motivating example, and show:

\vskip10pt

\begin{itemize}

\item How 4 Data Scientists might approach this problem and get different conclusions for the ``optimal'' treatment/action to implement. 

\item The different conclusions emerge as a result of slightly different interpretation of the business problem. 

\item When the goal is to implement the action that maximizes expected profit \emph{counterfactual}-based decision-making outperforms all other approaches. 
...
\begin{center}
EDC  $\rightarrow$ $\underset{I \in {0,1}}{\mathrm{argmax}}~E[\pi|I, M, V=1]$
\end{center}
